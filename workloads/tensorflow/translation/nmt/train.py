...
def train(hparams, scope=None, target_session=""):
  ...
  utils.print_out("start: " + str(start_time_mb))
  last_iter = False
  while global_step < num_train_steps and not last_iter:
    seconds_elapsed = (datetime.datetime.now() - start_time_mb).total_seconds()
    minutes_elapsed = seconds_elapsed / 60.0
    if max_duration is not None and minutes_elapsed >= max_duration:
      last_iter = True
    ### Run a step ###
    ...
    if last_iter or global_step - last_stats_step >= steps_per_stats:
      last_stats_step = global_step
    ...
    if last_iter or global_step - last_eval_step >= steps_per_eval:
      last_eval_step = global_step
    ...
    if last_iter or global_step - last_external_eval_step >= steps_per_external_eval:
      last_external_eval_step = global_step
  ...
  utils.print_out("end: " + str(datetime.datetime.now()))
  # Done training
  loaded_train_model.saver.save(
      train_sess,
      os.path.join(out_dir, "translate.ckpt"),
