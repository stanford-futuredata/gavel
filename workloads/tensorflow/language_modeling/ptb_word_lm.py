...
def run_epoch(session, model, eval_op=None, verbose=False, start_time_mb=0):
  ...
  num_words = iters * model.input.batch_size * max(1, FLAGS.num_gpus)
  return np.exp(costs / iters), num_words
  ...
  with tf.Graph().as_default():
    ...
    with sv.managed_session(config=config_proto) as session:
      ...
      for i in range(config.max_max_epoch):
        ...
        train_perplexity, num_words = run_epoch(session, m, eval_op=m.train_op,
                                                verbose=True,
