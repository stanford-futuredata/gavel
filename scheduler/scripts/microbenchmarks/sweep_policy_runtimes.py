...
def measure_runtime(cluster_spec_str, num_active_jobs, policy_name,
                    pruning_threshold, warm_start, oracle_throughputs,
    ...
    if args.warm_start:
        results = results[1:]
    print('%s,%d,%s,%f,%s,%f' % (cluster_spec_str, num_active_jobs, policy.name,
                                 pruning_threshold, warm_start,
                                 np.mean(results)))
...
def main(args):
    ...
    all_pruning_thresholds = args.pruning_thresholds
    warm_start_options = [False]
    if args.warm_start:
        warm_start_options.append(True)
        num_trials = max(2, args.num_trials)
    else:
        num_trials = args.num_trials

    oracle_throughputs = utils.read_all_throughputs_json(args.throughputs_file)

    print('Cluster spec,# Active Jobs,Policy,Pruning threshold,'
          'Warm start,Runtime')
    for cluster_spec in all_cluster_specs:
        for num_active_jobs in all_num_active_jobs:
            for policy in all_policies:
                for pruning_threshold in all_pruning_thresholds:
                    for warm_start_option in warm_start_options:
                        measure_runtime(cluster_spec, num_active_jobs, policy,
                                        pruning_threshold, warm_start_option,
                                        oracle_throughputs,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--pruning_thresholds', type=float,
                        nargs='+',
                        default=list(np.linspace(0.0, 0.8, num=9)),
                        help='List of pruning thresholds to sweep')
    parser.add_argument('-w', '--warm_start', action='store_true',
                         default=False, help='Toggle warm start flag')
