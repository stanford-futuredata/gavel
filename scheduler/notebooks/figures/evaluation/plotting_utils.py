...
def plot_metric_vs_inverse_lambda_different_num_sub_problems(
    logfile_paths,
    labels,
    v100s, p100s, k80s,
    policies, metric_fn,
    ...
    for policy in policies:
        ...
        input_job_rates = [3600.0 / x for x in lambdas]
        metrics = []
        for i, x in enumerate(relevant_logfile_paths):
            metrics.append(metric_fn(x[1]))
            if i % 10 == 9:
                print("Done with [%d/%d]" % (i+1, len(relevant_logfile_paths)))
        seeds = [x[2] for x in relevant_logfile_paths]
...
def plot_jct_cdf_different_num_sub_problems(logfile_paths,
                                            labels,
                                            v100s, p100s, k80s,
                                            max_input_job_rate,
                                            policies, all_num_sub_problems,
                                            min_job_id, max_job_id,
                                            partition=True,
                                            finish_time_fairness=False,
                                            output_directory=None):
    ...
    for l in lambdas:
        ...
        if finish_time_fairness:
            relevant_logfile_paths = list(reversed(prune(
                logfile_paths, v100s, p100s, k80s, "isolated",
                num_sub_problems_provided=True)))
            relevant_logfile_paths = [x for x in relevant_logfile_paths
                                      if x[0] == l and x[2] == 0
                                      and x[3] == 1]
                ...
                for j, (ax, jcts_segment) in enumerate(zip(axes, jcts)):
                    ...
                    if j == 0:
                        ...
                        if num_sub_problems == 1:
                            labels_in_legend.append("Original")
                        else:
                            labels_in_legend.append(labels[policy] + " (%d)" % num_sub_problems)
                counter += 1
