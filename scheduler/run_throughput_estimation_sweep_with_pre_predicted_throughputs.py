...
def emulate_with_timeout(experiment_id, policy_name, schedule_in_rounds,
                         ...
                         completion_algo, drop_prob, verbose):
    f = io.StringIO()
    lam_str = 'lam=%2f.log' % (lam)
...
def get_all_throughput_files(predicted_throughputs_dir):
    throughput_files = {}
    algo_dirs = os.listdir(predicted_throughputs_dir)
    for algo_dir in algo_dirs:
        _, algo = algo_dir.split('=')
        throughput_files[algo] = {}
        algo_dir = os.path.join(predicted_throughputs_dir, algo_dir)
        seed_dirs = os.listdir(algo_dir)
        for seed_dir in seed_dirs:
            _, seed = seed_dir.split('=')
            seed = int(seed)
            throughput_files[algo][seed] = {}
            seed_dir = os.path.join(algo_dir, seed_dir)
            json_files = os.listdir(seed_dir)
            for json_file in json_files:
                drop_prob =\
                    round(float(json_file.split('=')[1].strip('.json')), 2)
                json_file = os.path.join(seed_dir, json_file)
                throughput_files[algo][seed][drop_prob] = json_file
    return throughput_files

def main(args):
    if args.window_start >= args.window_end:
        raise ValueError('Window start must be < than window end.')

    all_throughput_files = get_all_throughput_files('predicted_throughputs')
    ...
    for ratio_str in args.ratios:
        ...
        for policy_name in policy_names:
            ...
            for completion_algo in args.completion_algorithms:
                ...
                for drop_prob in args.drop_probs:
                    drop_prob_str =\
                        'drop_prob_str=%f' % (drop_prob)
                    raw_logs_drop_prob_subdir =\
                        os.path.join(raw_logs_completion_algo_subdir,
                                     drop_prob_str)
                    if not os.path.isdir(raw_logs_drop_prob_subdir):
                        os.mkdir(raw_logs_drop_prob_subdir)
                    for throughput in throughputs:
                        ...
                        for seed in args.seeds:
                            ...
                            raw_logs_seed_subdir = \
                                os.path.join(raw_logs_drop_prob_subdir,
                                             seed_str)
                            if not os.path.isdir(raw_logs_seed_subdir):
                                os.mkdir(raw_logs_seed_subdir)

                            throughputs_file =\
                                all_throughput_files[completion_algo][seed][drop_prob]
...
if __name__=='__main__':
    ...
    parser.add_argument('--completion_algorithms', type=str, nargs='+',
                        ...
                        help=('Matrix completion algorithm for throughput '
                              'prediction'))
    parser.add_argument('--drop_probs', type=float, nargs='+',
                        default=[0.15, 0.25, 0.50, 0.75, 0.85],
                        help='Drop probabilities to sweep over')
